...
function okGeo(lat,lon) {
    document.getElementById('lon').value = lon;
    document.getElementById('lat').value = lat;
    $.getJSON('http://wouf.it/test.php?lon='+lon+'&lat='+lat+ '&elist=?');
}

// Timeout while getting geolocation
function errGeo() {
    if(document.getElementById('lang').value == 'fr') {
	alert("Nous n'avons pas pu vous g\351olocaliser. Merci de relancer.");
    } else {
	alert("We couldn't find your location. Please restart.");
    }
}
